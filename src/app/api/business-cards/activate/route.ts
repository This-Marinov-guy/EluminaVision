...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabase.rpc("find_business_card_by_partial_id", {
  ...
  try {
    const { error } = await supabase.from("business_cards").update({ status: 2, user_id: userId }).eq("id", data[0].id);
