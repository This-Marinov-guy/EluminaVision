...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabaseAdmin.rpc("find_business_card_by_partial_id", {
    partial_id: partialId,
  });  

  if (error || data.length === 0) {
    return NextResponse.json({ status: false, message: "Business Card not found or already activated" }, { status: 404 });
  }

  try {
    const { error } = await supabaseAdmin.from("business_cards").update({ status: 2, user_id: userId }).eq("id", data[0].id);
  ...
  return NextResponse.json({ status: true, message: "Business Card claimed successfully" }, { status: 200 });
}
